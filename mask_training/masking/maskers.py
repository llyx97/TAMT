    ...
    def replace(self, m, root_name, names_tobe_masked, masked_linear_cls):
        for attr_str in dir(m):
            ...
            if isinstance(target_attr, nn.Module):
                ...
                if hasattr(target_attr, 'weight') and not ('predictions' in name or 'lm_head' in name):
                    target_attr.weight.requires_grad = False
                if hasattr(target_attr, 'bias') and not ('predictions' in name or 'lm_head' in name):
