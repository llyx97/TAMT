...
def main():
    ...
    parser.add_argument("--output_mask_dir",
                        default=None,
                        type=str)
    parser.add_argument("--load_mask_dir",
                        default=None,
                        type=str)
    ...
    if args.structured:
        ...
        if args.load_mask_dir is not None:
            mask = torch.load(args.load_mask_dir, map_location=device)
            pruning_model_with_mask(student_model, mask)
            zero_rate = see_weight_rate(student_model)
