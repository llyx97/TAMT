...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_att=False, head_mask=None):
        mixed_query_layer = self.query(hidden_states)
        mixed_key_layer = self.key(hidden_states)
        mixed_value_layer = self.value(hidden_states)
        ...
        context_layer = torch.matmul(attention_probs, value_layer)
        head_mags = context_layer.norm(p=2, dim=-1).transpose(-1, -2)
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape = context_layer.size()[
            :-2] + (self.all_head_size,)
        context_layer = context_layer.view(*new_context_layer_shape)
        return context_layer, attention_scores, head_mags
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, head_mask):
        self_output, layer_att, head_mags = self.self(input_tensor, attention_mask, head_mask=head_mask)
        attention_output = self.output(self_output, input_tensor)
        return attention_output, layer_att, head_mags
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, head_mask, ffn_mask):
        attention_output, layer_att, head_mags = self.attention(
            hidden_states, attention_mask, head_mask=head_mask)
        intermediate_output = self.intermediate(attention_output, ffn_mask)
        layer_output = self.output(intermediate_output, attention_output)

        return layer_output, intermediate_output.abs(), head_mags
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, head_mask, ffn_mask):
        ...
        for i, layer_module in enumerate(self.layer):
            hidden_states, ffn_mags, head_mags = layer_module(
                hidden_states, attention_mask, head_mask=head_mask[i], ffn_mask=ffn_mask[i])
            all_ffn_mags.append(ffn_mags)
            all_head_mags.append(head_mags)

        return all_ffn_mags, all_head_mags
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                output_all_encoded_layers=True, output_att=True, head_mask=None, ffn_mask=None):
        ...
        if ffn_mask is not None:
            ...
            ffn_mask = [None] * self.config.num_hidden_layers
        ...
        ffn_mags, head_mags = self.encoder(embedding_output,
                                                  extended_attention_mask, head_mask=head_mask, ffn_mask=ffn_mask)
        ffn_mags = torch.stack(ffn_mags).mean(1).mean(1)
        head_mags = torch.stack(head_mags).mean(1).mean(1)
        return ffn_mags, head_mags
...
class TinyBertForPreTraining(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None,
                ...
                next_sentence_label=None, labels=None, is_student=False, head_mask=None, ffn_mask=None):
        sequence_output, att_output, pooled_output = self.bert(
            input_ids, token_type_ids, attention_mask, head_mask=head_mask, ffn_mask=ffn_mask)
        tmp = []
        if is_student:
