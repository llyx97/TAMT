	...
	do
		...
		do
			python $root_dir/mask_training/eval_kd.py \
				...
				--load_mask_dir $root_dir/mask_training/models/prun_bert/unstructured/$prun_type/wikitext-103/length$block_size/full_cosine/mag_init/$ZERO_RATE/seed$seed/step_$step/mask.pt  \
				--eval_batch_size 16 \
				--repr_distill true \
				--structured false
		done
