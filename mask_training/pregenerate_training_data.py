...
def create_instances_from_document(
        ...
        masked_lm_prob, max_predictions_per_seq, whole_word_mask, vocab_list, bi_text=True, tokenizer_type='bert'):
    ...
    while i < len(document):
        ...
        if i == len(document) - 1 or current_length >= target_seq_length:
            if current_chunk:
                ...
                if tokenizer_type=='bert':
                    tokens = ["[CLS]"] + tokens_a + ["[SEP]"] + tokens_b + ["[SEP]"] if len(tokens_b)>=1 else ["[CLS]"] + tokens_a + ["[SEP]"]
                elif tokenizer_type=='roberta':
                    tokens = ["<s>"] + tokens_a + ["</s>"] + ["</s>"] + tokens_b + ["</s>"] if len(tokens_b)>=1 else ["<s>"] + tokens_a + ["</s>"]
...
def create_training_file(docs, vocab_list, args, epoch_num, bi_text=True, tokenizer_type='bert'):
    ...
    with epoch_filename.open('w') as epoch_file:
        for doc_idx in trange(len(docs), desc="Document"):
            doc_instances = create_instances_from_document(
                ...
                whole_word_mask=args.do_whole_word_mask, vocab_list=vocab_list, bi_text=bi_text, tokenizer_type=tokenizer_type)
    ...
    with DocumentDatabase(reduce_memory=args.reduce_memory) as docs:
        with args.train_corpus.open() as f:
            ...
            for line in tqdm(f, desc="Loading Dataset", unit=" lines"):
                line = line.strip()
                if line == "":
                    if doc!=[] and (not '=' in doc[0]) and len(doc[0])>=20:
        ...
        if args.num_workers > 1:
            ...
            for epoch in trange(args.epochs_to_generate, desc="Epoch"):
                ...
                epoch_file, metric_file = create_training_file(docs, vocab_list, args, epoch, bi_text=bi_text, tokenizer_type=tokenizer_type)
