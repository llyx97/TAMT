...
def pruning_model(model, px, model_type):
    modules_to_prune  =[]
    for ii in range(12):
...
def rewind(pre_weight, model_type):
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    orig = rewind(model.state_dict(), args.model_type)

    print('starting pruning')
    pruning_model(model, 1/(10-pruning_step), args.model_type)
    rate_weight_equal_zero = see_weight_rate(model, args.model_type)
    pruning_step += 1
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    output_dir = os.path.join(args.output_dir, str(round((pruning_step+1)*0.1, 2)))
                    ...
                    rate_weight_equal_zero = see_weight_rate(model, args.model_type)
                    pruning_step += 1
                    print('zero_rate = %.10f at step %d'%(rate_weight_equal_zero, global_step))
                    tb_writer.add_scalar("zero_rate", rate_weight_equal_zero, global_step)
