...
from hg_transformers.data.metrics.squad_metrics import (
    compute_predictions_log_probs,
    compute_predictions_logits,
    squad_evaluate,
)
from hg_transformers.data.processors.squad import SquadResult, SquadV1Processor, SquadV2Processor
...
def pruning_model_custom(model, mask_dict, do_remove=False):
    ...
    for ii in range(12):
        ...
        modules_to_prune += ['bert.encoder.layer.%d.attention.self.query'%ii, \
                     'bert.encoder.layer.%d.attention.self.key'%ii, 'bert.encoder.layer.%d.attention.self.value'%ii,\
                     'bert.encoder.layer.%d.attention.output.dense'%ii, 'bert.encoder.layer.%d.intermediate.dense'%ii,\
                     'bert.encoder.layer.%d.output.dense'%ii]

    parameters_to_prune.append(model.bert.pooler.dense)
    parameters_to_prune.append(model.bert.embeddings.word_embeddings)
    modules_to_prune.append('bert.pooler.dense')
    modules_to_prune.append('bert.embeddings.word_embeddings')
...
def main():
    ...
    parser.add_argument(
        "--subset_size", default=-1, type=int, help="Number of samples in the sub training set..",
    )
    ...
    if args.do_train:
        ...
        if args.subset_size > 0:
            #train_dataset, _ = random_split(train_dataset, [args.subset_size, len(train_dataset)-args.subset_size], generator=torch.Generator().manual_seed(args.seed))
            train_dataset, _ = random_split(train_dataset, [args.subset_size, len(train_dataset)-args.subset_size])
            num_step_per_epoch = math.ceil(len(train_dataset) / args.per_gpu_train_batch_size)
            if args.logging_steps > num_step_per_epoch:
                args.logging_steps = num_step_per_epoch
                logger.info("First 5 samples in the subset:")
                for i in range(5):
                    logger.info(train_dataset[i][0])
